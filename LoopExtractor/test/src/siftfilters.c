...
int localizeIsWeak (ScaleSpace *this, KeyPoint *point)
{
	...
	while (needToAdjust) {
		...
		fv = localizeIsWeak_loop_1(&i, &rows, &adj);
...
Matrix *getAdjustment(ScaleSpace *this, KeyPoint *point, int level, double *dotP) {
	...
	*dotP = 0.0;
	...
	sum = 0.0;
	{
		...
		fv = getAdjustment_loop_2(&i, &result, &sum, &d);
	...
	*dotP = sum;
	...
	matrixCopy->values = (double **) malloc(matrixCopy->rows*sizeof(double *));
	{
		...
		fv = getAdjustment_loop_3(&i, &matrixCopy);
...
int isFingerTip(ScaleSpace *this, ScaleSpace *gaussian, KeyPoint *point) {
...

	{
		...
		fv = isFingerTip_loop_4(&i, &MAX_SEARCH, &point, &this, &val, &diffImage, &PEAK_DEF, edgeX, &foundEdges, edgeY, &angle);
...

	{
		...
		fv = isFingerTip_loop_5(&i, &MAX_SEARCH, &point, &this, &val, &diffImage, &PEAK_DEF, edgeX, &foundEdges, edgeY, &angle);
...

	{
		...
		fv = isFingerTip_loop_6(&i, &MAX_SEARCH, &point, &this, &val, &diffImage, &PEAK_DEF, edgeX, &foundEdges, edgeY, &angle);
...

	{
		...
		fv = isFingerTip_loop_7(&i, &MAX_SEARCH, &point, &this, &val, &PEAK_DEF, edgeX, &foundEdges, edgeY, &angle, &diffImage);
	...
	*/
	...
	//compute distances sq.
	{
		...
		fv = isFingerTip_loop_8(&i, &foundEdges, distance, edgeX, &point, edgeY);
	...
	if(foundEdges == 3) {
		...
		fv = isFingerTip_loop_9(&i, &foundEdges, &j, distance, &re_arg_pa1_9, &re_arg_pa2_9);
	...
	if(re_arg_pa1_9 != -1) return re_arg_pa2_9; 
		...
		fv = isFingerTip_loop_10(&i, &foundEdges, distance);
	...
	printf("  found edges for y=%d x=%d val=%f:\n", point->y, point->x, this->images[point->imageLevel]->pic[point->y][point->x]);
	{
		...
		fv = isFingerTip_loop_11(&i, &foundEdges);
