...
Image *getCannyEdgesClassic(Image *original, float sigma, float percent) {
		...
		return NULL;
	{
		...
		fv = getCannyEdgesClassic_loop_1(&i, &maskSize, &maskX, &maskY);
		...
		return NULL;
	{
		...
		fv = getCannyEdgesClassic_loop_2(&i, &original, &outpicX, &outpicY);
		...
		return NULL;
	{
		...
		fv = getCannyEdgesClassic_loop_3(&i, &outImage);
	...
	normalizeFactor = 1.0/(2.0*M_PI*sigma*sigma);
	{
		...
		fv = getCannyEdgesClassic_loop_4(&p, &mr, &q, &maskval, &normalizeFactor, &sigma, &maskX, &centY, &centX, &maskY);
	...
	/*convolve masks with picture*/
	{
		...
		fv = getCannyEdgesClassic_loop_5(&i, &mr, &original, &j, &sum1, &sum2, &p, &q, &maskX, &centY, &centX, &maskY, &outpicX, &outpicY);
	...
	/*compute strength/magnitude*/
	{
		...
		fv = getCannyEdgesClassic_loop_6(&i, &original, &j, &outImage, &mr, &outpicX, &outpicY);
	...
	max = 0.0;
	{
		...
		fv = getCannyEdgesClassic_loop_7(&i, &outImage, &j, &max);


}
	{
    
		fv = getCannyEdgesClassic_loop_8(&i, &outImage, &j, &max);
	...
	peaks->pic = (float **) malloc(peaks->height*sizeof(float *));
	{
		...
		fv = getCannyEdgesClassic_loop_9(&i, &peaks);
	...
	/*edge detection/peaks of ridges*/
	{
		...
		fv = getCannyEdgesClassic_loop_10(&i, &peaks, &j, &xDiff, &outpicX, &yDiff, &outpicY, &angle, &outImage);
	...
	/*compute high/low thresholds*/
	{
		...
		fv = getCannyEdgesClassic_loop_11(&i, &outImage, &j, hist);
	...
	picPercent = (int) (percent*outImage->width*outImage->height);
	{
		...
		fv = getCannyEdgesClassic_loop_12(&i, &sum, hist, &picPercent, &HI);
	...
	canny->pic = (float **) malloc(canny->height*sizeof(float *));
	{
		...
		fv = getCannyEdgesClassic_loop_13(&i, &canny);


}
	{
    
		fv = getCannyEdgesClassic_loop_14(&i, &canny, &j);
	...
	/* hysterisis/double thresholding */
	{
		...
		fv = getCannyEdgesClassic_loop_15(&i, &outImage, &j, &peaks, &HI, &canny, &LO);
...

	{
		...
		fv = getCannyEdgesClassic_loop_16(&i, &peaks, &j, &canny);
	...
	/* clear border pixels */
	{
		...
		fv = getCannyEdgesClassic_loop_17(&i, &mr, &j, &canny);


}
	{
    
		fv = getCannyEdgesClassic_loop_18(&i, &canny, &mr, &j);


}
	{
    
		fv = getCannyEdgesClassic_loop_19(&i, &canny, &j, &mr);


}
	{
    
		fv = getCannyEdgesClassic_loop_20(&i, &canny, &j, &mr);
	...
	/* deallocate memory */
	{
		...
		fv = getCannyEdgesClassic_loop_21(&i, &peaks, &outImage);
...
Image *getCannyEdges(Image *image, float sigma, float percent, Image *angleSource) {
	...
	if(angleSource == NULL) {
		...
		fv = getCannyEdges_loop_22(&i, &original, &j, &xDeriv, &yDeriv);
	...
	max = 0.0;
	{
		...
		fv = getCannyEdges_loop_23(&i, &original, &j, &max);


}
	{
    
		fv = getCannyEdges_loop_24(&i, &original, &j, &max);
	...
	peaks->pic = (float **) malloc(peaks->height*sizeof(float *));
	{
		...
		fv = getCannyEdges_loop_25(&i, &peaks);
	...
	/*edge detection/peaks of ridges*/
	{
		...
		fv = getCannyEdges_loop_26(&i, &peaks, &j, &angleSource, &xDiff, &xDeriv, &yDiff, &yDeriv, &angle, &original);
	...
	/*compute high/low thresholds*/
	{
		...
		fv = getCannyEdges_loop_27(&i, &original, &j, hist);
	...
	picPercent = (int) (percent*original->width*original->height);
	{
		...
		fv = getCannyEdges_loop_28(&i, &sum, hist, &picPercent, &HI);
	...
	canny->pic = (float **) malloc(canny->height*sizeof(float *));
	{
		...
		fv = getCannyEdges_loop_29(&i, &canny);


}
	{
    
		fv = getCannyEdges_loop_30(&i, &canny, &j);
	...
	/* hysterisis/double thresholding */
	{
		...
		fv = getCannyEdges_loop_31(&i, &original, &j, &peaks, &HI, &canny, &LO);
...

	{
		...
		fv = getCannyEdges_loop_32(&i, &peaks, &j, &canny);
	...
	/* deallocate memory */
	{
		...
		fv = getCannyEdges_loop_33(&i, &peaks, &original);
...
Canny *newCannyPPM(ImageRGB *image, double sigma) {
	...
	gaussianx = (double **) malloc(gSize*sizeof(double *));
	{
		...
		fv = newCannyPPM_loop_34(&i, &gSize, &gaussianx);
        ...
        gaussiany = (double **) malloc(gSize*sizeof(double *));
	{
		...
		fv = newCannyPPM_loop_35(&i, &gSize, &gaussiany);
	...
	//q and p switched
	{
		...
		fv = newCannyPPM_loop_36(&i, &gSize, &q, &j, &p, &gaussianx, &kappa, &sigma, &gaussiany);
	...
	/* convolve */
	{
		...
		fv = newCannyPPM_loop_39(&y, &new, &x, &dgxSum, &dgySum, &xrSum, &yrSum, &xgSum, &ygSum, &xbSum, &ybSum, &i, &gSize, &q, &j, &p, &image, &gaussianx, &gaussiany, &rMag, &gMag, &bMag);
	...
	max = 0.0;
	{
		...
		fv = newCannyPPM_loop_40(&i, &new, &j, &max);


}
	{
    
		fv = newCannyPPM_loop_41(&i, &new, &j, &max);
	...
	/* deallocate gaussianx, gaussiany */
	{
		...
		fv = newCannyPPM_loop_42(&i, &gSize, &gaussianx, &gaussiany);
	...
	peaks = (int **) malloc(new->height*sizeof(int *));
	{
		...
		fv = newCannyPPM_loop_43(&i, &new, &peaks);
	...
	// iterate through the gradient image
	{
		...
		fv = newCannyPPM_loop_44(&y, &new, &x, &peaks, &sector);
	...
	/* eliminate low values */
	{
		...
		fv = newCannyPPM_loop_45(&y, &new, &x, &peaks, &low);
	...
	/* keep high values */
	{
		...
		fv = newCannyPPM_loop_46(&y, &new, &x, &peaks, &high);
	...
	traced = (int **) malloc(new->height*sizeof(int *));
	{
		...
		fv = newCannyPPM_loop_47(&i, &new, &traced);


}
	{
    
		fv = newCannyPPM_loop_48(&i, &new, &j, &traced);
        ...
        //   the recursion there
	{
		...
		fv = newCannyPPM_loop_49(&y, &new, &x, &traced, &low, &peaks);
	...
	border = (int) (3*new->sigma);
	{
		...
		fv = newCannyPPM_loop_50(&j, &border, &i, &new);


}
	{
    
		fv = newCannyPPM_loop_51(&j, &new, &border, &i);


}
	{
    
		fv = newCannyPPM_loop_52(&i, &border, &j, &new);


}
	{
    
		fv = newCannyPPM_loop_53(&i, &new, &border, &j);
	...
	/* deallocate peaks and traced arrays */
	{
		...
		fv = newCannyPPM_loop_54(&i, &new, &peaks, &traced);
...
Canny  *newCannyPGMTwoSources(Image *gradient, Image *gaussian) {
	...
	new->magnitude = (double **) malloc(new->height*sizeof(double *));
	{
		...
		fv = newCannyPGMTwoSources_loop_55(&i, &new);
	...
	new->theta = (double **) malloc(new->height*sizeof(double *));
	{
		...
		fv = newCannyPGMTwoSources_loop_56(&i, &new);
	...
	/* initialize magnitudes and thetas */
	{
		...
		fv = newCannyPGMTwoSources_loop_57(&i, &new, &j, &gradient, &gaussian);
	...
	peaks = (int **) malloc(new->height*sizeof(int *));
	{
		...
		fv = newCannyPGMTwoSources_loop_58(&i, &new, &peaks);
	...
	/* find peaks */
	{
		...
		fv = newCannyPGMTwoSources_loop_59(&y, &new, &x, &peaks, &sector);
	...
	/* eliminate low values */
	{
		...
		fv = newCannyPGMTwoSources_loop_60(&y, &new, &x, &peaks, &low);
	...
	/* keep high values */
	{
		...
		fv = newCannyPGMTwoSources_loop_61(&y, &new, &x, &peaks, &high);
	...
	traced = (int **) malloc(new->height*sizeof(int *));
	{
		...
		fv = newCannyPGMTwoSources_loop_62(&i, &new, &traced);


}
	{
    
		fv = newCannyPGMTwoSources_loop_63(&i, &new, &j, &traced);
        ...
        //   the recursion there
	{
		...
		fv = newCannyPGMTwoSources_loop_64(&y, &new, &x, &traced, &low, &peaks);
	...
	/* deallocate peaks and traced arrays */
	{
		...
		fv = newCannyPGMTwoSources_loop_65(&i, &new, &peaks, &traced);
...
void findThresholds(Canny *this, double *low, double *high) {
	...
	// fill the histogram
	{
		...
		fv = findThresholds_loop_66(&i, &this, &j, hist);
        ...
        //   finish
	{
		...
		fv = findThresholds_loop_67(&c, &area, hist, &percent, &this);
