...
int localizeIsWeak_loop_1(int *i, int *rows, Matrix * *adj)
{
for((*i) = 0; (*i) < (*rows); ++(*i)) {
			free((*adj)->values[(*i)]);
		}
...
int getAdjustment_loop_2(int *i, gsl_vector * *result, double *sum, gsl_vector * *d)
{
...
int getAdjustment_loop_3(int *i, Matrix * *matrixCopy)
{
...
int isFingerTip_loop_4(int *i, const int *MAX_SEARCH, KeyPoint * *point, ScaleSpace * *this, double *val, Image * *diffImage, const float *PEAK_DEF, int edgeX[], int *foundEdges, int edgeY[], double *angle)
{
...
int isFingerTip_loop_5(int *i, const int *MAX_SEARCH, KeyPoint * *point, ScaleSpace * *this, double *val, Image * *diffImage, const float *PEAK_DEF, int edgeX[], int *foundEdges, int edgeY[], double *angle)
{
for((*i) = 1; (*i) < (*MAX_SEARCH); ++(*i)) {
		if((*point)->y - (*i) - 1 < 0 ||
		   (*point)->x < 1 ||
...
int isFingerTip_loop_6(int *i, const int *MAX_SEARCH, KeyPoint * *point, ScaleSpace * *this, double *val, Image * *diffImage, const float *PEAK_DEF, int edgeX[], int *foundEdges, int edgeY[], double *angle)
{
for((*i) = 1; (*i) < (*MAX_SEARCH); ++(*i)) {
		if((*point)->x - (*i) - 1< 0 ||
		   (*point)->y < 1 ||
...
int isFingerTip_loop_7(int *i, const int *MAX_SEARCH, KeyPoint * *point, ScaleSpace * *this, double *val, const float *PEAK_DEF, int edgeX[], int *foundEdges, int edgeY[], double *angle, Image * *diffImage)
{
for((*i) = 1; (*i) < (*MAX_SEARCH); ++(*i)) {
...
int isFingerTip_loop_8(int *i, int *foundEdges, double distance[], int edgeX[], KeyPoint * *point, int edgeY[])
{
...
int isFingerTip_loop_9(int *i, int *foundEdges, int *j, double distance[], int *re_arg_pa1_9, int *re_arg_pa2_9)
{
for((*i) = 0; (*i) < (*foundEdges); ++(*i)) {
			for((*j) = 0; (*j) < (*foundEdges); ++(*j)) {
				if(distance[(*i)] > 4*distance[(*j)]) {
					...
					{(*re_arg_pa1_9) = 0; (*re_arg_pa2_9) = 0; return 0;}
				}
			}
		}
...
int isFingerTip_loop_10(int *i, int *foundEdges, double distance[])
{
...
int isFingerTip_loop_11(int *i, int *foundEdges)
{
for((*i) = 0; (*i) < (*foundEdges); ++(*i)) {
