...
Image *allocateImage(int width, int height) {
	...
	{	 int re_arg_pa1_1 = -1; Image * re_arg_pa2_1;
		...
		fv = allocateImage_loop_1(&i, &new, &j, &re_arg_pa1_1, &re_arg_pa2_1);
	...
	/* initialize */
	{
		...
		fv = allocateImage_loop_2(&i, &new, &j);
...
Image *allocateImageFromDoubles(double **array, int width, int height) {
	...
	new = allocateImage(width, height);
	{
		...
		fv = allocateImageFromDoubles_loop_3(&i, &new, &j, &array);
...
Image *allocateImageFromInts(int **array, int maxValue, int width, int height) {
	...
	new = allocateImage(width, height);
	{
		...
		fv = allocateImageFromInts_loop_4(&i, &new, &j, &array, &maxValue);
...
Image *halveImage(Image *original)
{
	...
	halved = allocateImage(original->width/2, original->height/2);
	{
		...
		fv = halveImage_loop_6(&i, &halved, &j, &original);
...
Image *doubleImage(Image *original)
{
	...
	// be filled with 0
	{
		...
		fv = doubleImage_loop_7(&i, &original, &j, &doubled);
...
Image *doubleCanny(Image *original) {
	...
	doubled->pic = (float **) malloc(doubled->height*sizeof(float *));
	{
		...
		fv = doubleCanny_loop_8(&i, &doubled);
	...
	//copy exisiting pixels
	{
		...
		fv = doubleCanny_loop_9(&i, &original, &j, &doubled);
	...
	// fill new pixels
	{
		...
		fv = doubleCanny_loop_10(&i, &original, &j, &doubled);
...
Image *subtractImages(Image *one, Image *two) {
	...
	/* subtract */
	{
		...
		fv = subtractImages_loop_11(&i, &one, &j, &newImage, &two);
...
Image *cloneImage(Image *original) {
	...
	/* copy values */
	{
		...
		fv = cloneImage_loop_12(&i, &newImage, &j, &original);
...
float** getDerivativeX(Image *original) {
	...
	imageX = (float **) malloc(original->height*sizeof(float *));
	{
		...
		fv = getDerivativeX_loop_13(&i, &original, &imageX);
	...
	
	{
		...
		fv = getDerivativeX_loop_14(&i, &original, &j, &sum1, &p, maskX, &imageX);
...
float** getDerivativeY(Image *original) {
	...
	imageY = (float **) malloc(original->height*sizeof(float *));
	{
		...
		fv = getDerivativeY_loop_15(&i, &original, &imageY);
	...
	
	{
		...
		fv = getDerivativeY_loop_16(&j, &original, &i, &sum2, &p, maskY, &imageY);
...
Image *getDerivative(Image *original) {
	...
	/* compute gradient */
	{
		...
		fv = getDerivative_loop_17(&i, &original, &j, &newImage, &imageX, &imageY);
	...
	/* deallocate storage */
	{
		...
		fv = getDerivative_loop_18(&i, &original, &imageX, &imageY);
...
int isPeak(Image *image, int x, int y, int direction) {
	...
	{	 int re_arg_pa1_19 = -1; int re_arg_pa2_19;
		...
		fv = isPeak_loop_19(&i, &y, &j, &x, &image, &yIncr, &xIncr, &peakVal, &peakR, &re_arg_pa1_19, &re_arg_pa2_19);
	...
	{	 int re_arg_pa1_20 = -1; int re_arg_pa2_20;
		...
		fv = isPeak_loop_20(&i, &y, &j, &x, &image, &yIncr, &xIncr, &peakVal, &peakL, &re_arg_pa1_20, &re_arg_pa2_20);
...
void drawLine(Image *image, int y1, int x1, int y2, int x2) {
	...
	if(ABS(y2 - y1) < ABS(x2 - x1)) {
		...
		fv = drawLine_loop_21(&i, &x1, &x2, &comp, &y1, &dy, &dx, &image);
		...
		fv = drawLine_loop_22(&i, &y1, &y2, &comp, &x1, &dx, &dy, &image);
...
void exportCSV(Image *image, char *fileName) {
	...
	output = fopen(fileName, "w");
	{
		...
		fv = exportCSV_loop_23(&i, &image, &j, &output);
...
float findMin(Image *image) {
...

	{
		...
		fv = findMin_loop_24(&i, &image, &j, &min);
...
float findMax(Image *image) {
...

	{
		...
		fv = findMax_loop_25(&i, &image, &j, &max);
...
ImageRGB *convertToRGB(Image *image) {
	...
	new = allocateImageRGB(image->width, image->height);
	{
		...
		fv = convertToRGB_loop_26(&i, &image, &j, &new);
