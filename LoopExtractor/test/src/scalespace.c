...
ScaleSpace *buildScaleSpaceOctave(Image *original, double imgScale, double imageSigma) {
	...
	sigma = START_SIGMA;
	{
		...
		fv = buildScaleSpaceOctave_loop_1(&i, &space, &sigma, &k);
...

	{
		...
		fv = buildScaleSpaceOctave_loop_2(&i, &space, &sourceBlur, &imageSigma, &targetBlur, &original, &sigma);
...
ScaleSpace *buildDifferences(ScaleSpace *original) {
		...
		return NULL;
	{
		...
		fv = buildDifferences_loop_3(&i, &space, &original);
	...
	space->processedLocalization = (int **) malloc(space->imgHeight*sizeof(int *));
	{
		...
		fv = buildDifferences_loop_4(&i, &space);


}
	{
    
		fv = buildDifferences_loop_5(&i, &space, &j);
...
ScaleSpace *buildScaleSpaceEdges(ScaleSpace *original) {
		...
		return NULL;
	{
		...
		fv = buildScaleSpaceEdges_loop_6(&i, &space, &original);
...
ScaleSpace *buildScaleSpaceCanny(ScaleSpace *gaussian, ScaleSpace *derivative) {
		...
		return NULL;
	{
		...
		fv = buildScaleSpaceCanny_loop_7(&i, &space, &derivative, &gaussian);
...
KeyPoint *findPeaks(ScaleSpace *this, Image *current, Image *above, Image *below, int DoGlevel) {
	...
	/* check if minimum or maximum */
	{
		...
		fv = findPeaks_loop_8(&i, &current, &j, &min, &isMin, &p, &q, &above, &below, &temp, &DoGlevel, &this, &peaks, &max, &isMax);
...
void printHistogram(ScaleSpace *this, KeyPoint *point) {
	...
	/* build the histogram */
	{
		...
		fv = printHistogram_loop_9(&y, &yMin, &yMax, &x, &xMin, &xMax, &relativeX, &point, &relativeY, &radius, &yDiff, &this, &xDiff, &direction, &binIndex, &oneBinRad, &magnitude, bins, &sigma);
	...
	maxGradient = 0.0;
	{
		...
		fv = printHistogram_loop_10(&binIndex, bins, &maxGradient);
	...
	fprintf(histOutput2, "Pt. %d %d, Level %d\n", (int) (point->y), (int) (point->x), point->imageLevel);
	{
		...
		fv = printHistogram_loop_11(&binIndex, &barWidth, bins, &maxGradient, &i);
...
void getMagnitudesAndOrientations(ScaleSpace *this) {
	...
	imageLevels = this->imageCount - 2;
	{
		...
		fv = getMagnitudesAndOrientations_loop_12(&i, &height, &j, &width, &k, &imageLevels, &xDiff, &this, &yDiff);
...
double ***allocate3D(int height, int width, int depth) {
	...
	}
	{
		...
		fv = allocate3D_loop_13(&i, &height, &matrix, &width);


}
	{
    
		fv = allocate3D_loop_14(&i, &height, &j, &width, &matrix, &depth);
...
void deallocate3D(double ***matrix, int height, int width) {
...

	{
		...
		fv = deallocate3D_loop_15(&i, &height, &j, &width, &matrix);
...
KeyPoint *generateKeypoints(ScaleSpace *this, KeyPoint *peaks) {
...

	{
		...
		fv = generateKeypoints_loop_16(&current, &peaks, &this, &multipleKeypoints);
...
KeyPoint *generateKeypointSingle(ScaleSpace *this, KeyPoint *list, KeyPoint *point, int *extraCount) {
	...
	/* build the histogram */
	{
		...
		fv = generateKeypointSingle_loop_17(&y, &yMin, &yMax, &x, &xMin, &xMax, &relativeX, &point, &relativeY, &radius, &this, &oneBinRad, bins, &sigma);
	...
	maxGradient = 0.0;
	{
		...
		fv = generateKeypointSingle_loop_18(&binIndex, bins, &maxGradient);
	...
	*/
	...
	maxBin = 0;
	{
		...
		fv = generateKeypointSingle_loop_19(&binIndex, bins, &maxGradient, &maxBin);
	...
	*/
	...
	/* find other keypoints */
	{
		...
		fv = generateKeypointSingle_loop_20(&binIndex, &maxBin, isKeypoint, bins, &maxGradient, &left, &right);
...

	{
		...
		fv = generateKeypointSingle_loop_21(&binIndex, isKeypoint, &left, &right, bins, &maxDegreeCorrection, &maxGradient, &degree, &oneBinRad, &point, &extraCount, &newKey, &lastAddition, &list);
...
void createDescriptors(ScaleSpace *this, KeyPoint *list) {
	...
	xySpacing = 4.0; //each descriptor bin covers 4x4 pixel area
	{
		...
		fv = createDescriptors_loop_22(&current, &list, &angle, &descriptorDim, &directionCount, &y, &radius, &x, &imageX, &imageY, &this, &yR, &xR, &dir, &i, &j, &k, &magnitudeWeight, &sigma, &iy, &ix, &id, &binX, &binY, &binDir, &xDist, &xySpacing, &yDist, &dirDist, &dirSpacing);
...
float ***allocateFeatureVector(int descriptorDim, int directionCount) {
	...
	}
	{
		...
		fv = allocateFeatureVector_loop_23(&i, &descriptorDim, &matrix);


}
	{
    
		fv = allocateFeatureVector_loop_24(&i, &descriptorDim, &j, &matrix, &directionCount);
	...
	/* initialize */
	{
		...
		fv = allocateFeatureVector_loop_25(&i, &descriptorDim, &j, &k, &directionCount, &matrix);
...
void capAndNormalizeFV(KeyPoint *this) {
	...
	norm = 0.0;
	{
		...
		fv = capAndNormalizeFV_loop_26(&i, &this, &j, &k, &norm);
	...
	norm = sqrt(norm);
	{
		...
		fv = capAndNormalizeFV_loop_27(&i, &this, &j, &k, &norm);
	...
	/* cap */
	{
		...
		fv = capAndNormalizeFV_loop_28(&i, &this, &j, &k);
	...
	norm = 0.0;
	{
		...
		fv = capAndNormalizeFV_loop_29(&i, &this, &j, &k, &norm);
	...
	norm = sqrt(norm);
	{
		...
		fv = capAndNormalizeFV_loop_30(&i, &this, &j, &k, &norm);
...
void printKeyPoint(KeyPoint *this, FILE *out) {
...

	{
		...
		fv = printKeyPoint_loop_31(&i, &this, &j, &k, &out, &count);
...
void markImage(KeyPoint *list, Image* image) {
...

	{
		...
		fv = markImage_loop_32(&current, &list, &length, &degree, &endX, &endY, &image, &arrowX, &arrowY);
...
void markImageSPoint(KeyPoint *list, Image* image, double scale) {
...

	{
		...
		fv = markImageSPoint_loop_33(&current, &list, &image, &scale);
