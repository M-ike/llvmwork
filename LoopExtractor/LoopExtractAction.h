...
public:
    ...
    bool VisitGotoStmt(GotoStmt *RS);
...
public:
    LoopExtractorConsumer(StringRef InFile,CompilerInstance &CI) : PP(CI.getPreprocessor()),InputFile(InFile.str()) { 
        ...
        //条件编译头文件相关的数组
        ConditionCompileLoc.clear();
        ConditionCompileContent.clear();
        ConditionCompileContent1.clear();
        NameToMacroContent.clear();
        MacroNameVec.clear();
    }
...
private:
    ...
    //保存MacroContent
    std::map<int,std::pair<std::string,std::string> > MacroContentInFile;
    ...
    //保存条件编译的内容
    std::map<int,int> ConditionCompileLoc;
    std::map<int,std::pair<std::string,std::string> > ConditionCompileContent;
    std::map<int,std::string> IncludeContent;
    std::map<int,std::string> ConditionCompileContent1; //保存着放在源文件中的内容.
    std::vector<std::string> MacroNameVec;
    std::map<std::string,std::string> NameToMacroContent;
    std::vector<int> includeLoc;
